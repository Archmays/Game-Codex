...
describe('Hanzi study tool contracts',()=>{
 ...
 it('is a lazy independent tool without inflating active game counts',()=>{
   expect(GAME_PORTFOLIO.find(r=>r.id==='hanzi-stroke-lab')).toMatchObject({definitionRole:'independent-tool',activeChildProduct:false,classicCardVisible:false,loadingPolicy:'route-lazy'});expect(ACTIVE_CHILD_PRODUCTS).toHaveLength(5);expect(CLASSIC_CARD_PRODUCTS).toHaveLength(5);
