...
root=Path(__file__).resolve().parents[2];staging=root/'tmp/tasks/oddity-v010/production-final';staging.mkdir(parents=True,exist_ok=True)
...
(staging/'web/build-identity.json').write_text(json.dumps({'product':'Game-Codex offline hub','oddityVersion':'0.1.0','sourceTree':freeze['sha256']},indent=2)+'\n',encoding='utf8')
(staging/'runtime').mkdir(exist_ok=True);shutil.copy2(shutil.which('node'),staging/'runtime/node.exe')
shutil.copy2(root/'tmp/tasks/oddity-v010/node-LICENSE',staging/'runtime/LICENSE')
...
if (root/'docs/oddity-puzzles/report.md').exists():shutil.copy2(root/'docs/oddity-puzzles/report.md',staging/'测试与边界.md')
forbidden=[p for p in staging.rglob('*') if p.suffix=='.blend'];assert not forbidden
dest=root/'handoffs/oddity-puzzles-v0.1.0-offline.zip';dest.parent.mkdir(exist_ok=True)
