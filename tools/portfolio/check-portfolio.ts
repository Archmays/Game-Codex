...
export function validatePortfolio(catalog: readonly GameCatalogMetadata[], currentCatalog: readonly GameCatalogMetadata[], root = resolve(import.meta.dirname, "../..")): string[] {
  ...
  const expectedActiveProducts = ["hanzi-tower-defense", "hanzi-word-adventure", "math-lab"];
  if (JSON.stringify(ACTIVE_CHILD_PRODUCTS.map((record) => record.id).sort()) !== JSON.stringify(expectedActiveProducts)) issues.push("Active child product truth must contain the three current products");
  if (JSON.stringify(CLASSIC_CARD_PRODUCTS.map((record) => record.id).sort()) !== JSON.stringify(expectedActiveProducts)) issues.push("Classic Hall must project only the three current products");
