...
describe("game portfolio governance", () => {
  ...
  it("represents the live catalog one-to-one with valid governance identities", () => {
    ...
    expect(GAME_PORTFOLIO.map(record => record.id)).toEqual(["oddity-puzzles", "world-in-a-box", "hanzi-stroke-lab", "hanzi-word-adventure", "hanzi-tower-defense", "equation-slider", "math-lab", "make-target", "memory-card"]);
