...
export function validatePortfolio(catalog: readonly GameCatalogMetadata[], currentCatalog: readonly GameCatalogMetadata[], root = resolve(import.meta.dirname, "../..")): string[] {
  const issues: string[] = [];
  const expectedIds = ["oddity-puzzles", "world-in-a-box", "hanzi-stroke-lab", "english-study", "hanzi-word-adventure", "hanzi-tower-defense", "equation-slider", "math-lab", "make-target", "memory-card"].sort();
  ...
  for (const record of GAME_PORTFOLIO) {
    ...
    if (record.contentStatus !== (["oddity-puzzles", "world-in-a-box", "hanzi-stroke-lab", "english-study", "hanzi-tower-defense", "hanzi-word-adventure"].includes(record.id) ? "playable" : "frozen")) issues.push(`${record.id} content status differs from the authorized new-game / protected-math boundary`);
  ...
  const expectedActiveProducts = ["hanzi-tower-defense", "hanzi-word-adventure", "math-lab", "oddity-puzzles", "world-in-a-box"];
  if (JSON.stringify(ACTIVE_CHILD_PRODUCTS.map((record) => record.id).sort()) !== JSON.stringify(expectedActiveProducts)) issues.push("Active child product truth must contain the five current products");
  if (JSON.stringify(CLASSIC_CARD_PRODUCTS.map((record) => record.id).sort()) !== JSON.stringify(expectedActiveProducts)) issues.push("Classic Hall must project only the five current products");
  ...
  if (PRIMARY_PLAY_SURFACES.length !== 7) issues.push(`Expected seven primary first-use surfaces, found ${PRIMARY_PLAY_SURFACES.length}`);
