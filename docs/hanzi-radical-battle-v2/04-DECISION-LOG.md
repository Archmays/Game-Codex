...
## D-031 第一章 M5 冻结为三地区、九行为、四首领和恢复式结尾

日期：2026-08-14
决定：第一章正式世界冻结为微光林径、花园回声、风的脚印三个区域与墨王核心；每区 12 字池、两条安全路线、两个普通节点、一个两阶段守护者、一次三选一和一个营地回报。九种普通行为各有 telegraph、effect、guaranteed recovery；最终字核三阶段只复用本局必见行为并以完整汉字发出恢复魔法。
为什么：完整章节需要区域差异、可重复路线和可辨高潮，但不能让怪物机制改写汉字答案、制造死路、引入惩罚性进度或把恢复叙事变成恐吓式击败。
儿童价值：三片区域的场景、同伴、路径、守护者和营地变化形成清楚的冒险起承转合；结束后可回营、自由冒险或翻魔法书，没有倒计时回访奖励。
汉字学习价值：所有干扰只改变当前操作环境；完整字、固定拼音、熟悉词、真实结构、正确部件与槽位保持不变量。首领组合已经见过的动作，最终一击明确来自完整汉字。
依据：`m5-content.ts`、schema-v3 run generator/machine、九行为与四首领单测、30,000-seed-ready scanner、18 条完整浏览器矩阵以及 story/free deterministic replay。
明确没有做什么：没有 Godot、后端账号、云端儿童追踪、每日登录、连胜、排行榜、抽卡、FOMO、永久损失、分数、正确率或羞辱性失败语言。
何时重新评估：仅当第一章既定范围出现软锁、答案改写、不可恢复行为、内容错误或儿童可用性缺陷；任何第二章需新授权。

## D-032 Theme C 正式素材只承担世界语义，汉字与拼音继续由代码渲染

日期：2026-08-14
决定：第一章使用 72 个选定、优化后的 Theme C WebP 运行时资产，覆盖大厅、四场景、三英雄、九怪物、四首领、24 个新字义魔法、18 能力、八修复与结尾；V1 十二字既有正式字义图保持冻结。生成图不包含正式汉字、拼音、答案或部件槽位，所有学习文字由浏览器 CJK 字体层渲染。
为什么：批量图像可以让世界、角色和反馈形成一致的儿童安全视觉系统，但生成图不适合承担规范字形、结构或读音真源。
依据：完整 production prompt pack、五张 selected atlas、处理脚本、72 项 runtime manifest、1,657,346 字节总预算、资产存在/尺寸门禁、11 个固定视觉/ARIA baseline 和人工画面检查。第一张修复 atlas 因语义不匹配被明确拒绝且不进入 runtime/package。
儿童价值：区域、英雄、守护者、能力和营地回报具有一致世界感；怪物非恐怖，静音与减少动画时仍能理解。
汉字学习价值：图像只表现字义联想与世界结果；完整字、部件与结构位置不被装饰遮挡，魔法书明确标注“不是字源说明”。
何时重新评估：仅当正式资产出现错绑、404、预算、低对比、误导性文字或明显风格破裂；不因个人审美偏好扩建资产范围。

## D-033 V2.0 本地生命周期与发布入口固定为 canonical 第一章

日期：2026-08-14
决定：大厅“汉字魔法战”主卡片与本地启动器进入 `?play=hanzi-v2-chapter-one&from=hub`；冻结 V1 继续保留 `?play=hanzi-v2-v1&from=hub`。双击启动器优先使用 5186，在 5186–5195 自动选择空闲端口，只复用/停止 PID、启动时间、仓库根、Vite 命令与端口完全匹配的记录进程。
为什么：家庭使用需要无需命令行的可靠入口，同时必须避免端口冲突时误杀用户的其他服务。V1 旧路由仍是不可改写历史兼容边界。
依据：launcher lifecycle 的 start、HTTP ready、exact PID reuse、owned cleanup 全部 PASS；浏览器矩阵、hub/spellbook/legacy/mobile surface 和 Pages base-path build 路径共同验证 canonical/legacy 分流。
儿童价值：双击后直接进入完整墨迹森林，儿童卡片不显示版本、年龄或学习指标。
汉字学习价值：启动器与 Pages 都挂载同一 36 字 runtime 与存档真源，不建立复制内容。
何时重新评估：canonical route、Vite 基础路径、本地存档 origin 或发布平台发生变化时；端口/PID 身份门禁不得降低。
