...
export function validatePortfolio(catalog: readonly GameCatalogMetadata[], currentCatalog: readonly GameCatalogMetadata[], root = resolve(import.meta.dirname, "../..")): string[] {
  const issues: string[] = [];
  const expectedIds = ["oddity-puzzles", "world-in-a-box", "hanzi-stroke-lab", "hanzi-word-adventure", "hanzi-tower-defense", "equation-slider", "math-lab", "make-target", "memory-card"].sort();
  ...
  for (const record of GAME_PORTFOLIO) {
    ...
    if (record.contentStatus !== (["oddity-puzzles", "world-in-a-box", "hanzi-stroke-lab", "hanzi-tower-defense", "hanzi-word-adventure"].includes(record.id) ? "playable" : "frozen")) issues.push(`${record.id} content status differs from the authorized new-game / protected-math boundary`);
