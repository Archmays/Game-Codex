...
async driver=>{
 ...
 try{await p.goto('http://127.0.0.1:5175/');return await p.evaluate(async()=>{
  ...
  const railSamples=[];for(const x of [-1.23,-.77]){ray.set(new T.Vector3(x,5,-4),new T.Vector3(0,-1,0));ray.far=6;const hit=ray.intersectObjects(meshes,false)[0];if(!hit||!hit.object.material.name.includes('Walnut'))throw Error('Bridge deck hides running rail');railSamples.push({x,height:hit.point.y});}
