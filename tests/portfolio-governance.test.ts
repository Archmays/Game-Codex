...
describe("game portfolio governance", () => {
  ...
  it("represents the live catalog one-to-one with valid governance identities", () => {
    ...
    expect(GAME_PORTFOLIO.map(record => record.id)).toEqual(["world-in-a-box", "hanzi-stroke-lab", "hanzi-word-adventure", "hanzi-tower-defense", "equation-slider", "math-lab", "make-target", "memory-card"]);
  ...
  it("binds the terminal lifecycle, first-use surfaces, and exact save inventory", () => {
    ...
    expect(PRIMARY_PLAY_SURFACES.map((surface) => surface.id)).toEqual([
      "world-in-a-box", "hanzi-word-adventure", "my-game-world", "classic-hub", "hanzi-tower-defense", "math-world",
    ]);
