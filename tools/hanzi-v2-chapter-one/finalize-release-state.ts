...
export function finalizeReleaseState(): void {
  ...
  requireValue(v1Tag === expectedV1 && v2Tag === head, "Release tags do not target the required commits");
  requireValue(statusPorcelain === "", `Repository is not clean: ${statusPorcelain}`);

  const generatedAtUtc = new Date().toISOString();
  const gitState = { schemaVersion: 1, sourceTreeSha256, branch, head, originMain, remoteMain, v1Tag: { name: "hanzi-magic-v2-v1.0.0", target: v1Tag }, v2Tag: { name: "hanzi-magic-v2-v2.0.0", target: v2Tag }, statusPorcelain, repoClean: true, generatedAtUtc };
