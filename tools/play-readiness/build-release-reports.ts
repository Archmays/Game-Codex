...
writeJson("FIRST_USE_AUDIT.json", { taskId: TASK_ID, verdict: "PASS", viewports: firstUseProjects, primaryEntries: 8, adultMetadataCount: 0, primaryActionVisible: true, evidence: firstUse });
...
writeJson("GIT_STATE.json", { taskId: TASK_ID, observedBaseline: "759d1a62a6aa9b2967ce2ee797a9dc8c28c23543", actualBaseline: "759d1a62a6aa9b2967ce2ee797a9dc8c28c23543", productTagCommit: commit, finalMainCommit: commit, originMain, tag: "game-codex-play-ready-v1.0.0", sourceTreeSha256 });
...
writeJson("FINAL_RESULT.json", { result: "PASS_MACHINE / PORTFOLIO_PLAY_READY / STABLE_FOR_NATURAL_FAMILY_USE", taskId: TASK_ID, commit, portfolioRecords: GAME_PORTFOLIO.length, classicStandalone: GAME_PORTFOLIO.filter((record) => record.currentStandaloneVisible).length, playSurfaces: PLAY_SURFACE_MANIFEST.length, knownSaveKeys: KNOWN_SAVE_KEYS.length, realChildValidation: "NOT_PERFORMED_AND_NOT_CLAIMED", naturalUseObservation: "PENDING_REAL_EVIDENCE" });
writeFileSync(resolve(REPORTS, "SOURCE_TREE_SHA256.txt"), `${sourceTreeSha256}  git-ls-tree-r-full-tree-HEAD\n`, "utf8");
writeFileSync(resolve(REPORTS, "FINAL_SUMMARY.md"), `# Game-Codex Play Readiness 05\n\nResult: **PASS_MACHINE / PORTFOLIO_PLAY_READY / STABLE_FOR_NATURAL_FAMILY_USE**\n\nThree released worlds and six Classic entries now converge on generated lifecycle truth, a 42-surface route inventory, child-facing first use, coherent return/resume behavior, semantic loading/error recovery, WCAG 2.2 relevant AA checks, sampled performance evidence, and a local Parent Save Vault covering 37 known keys (36 exportable).\n\nThis is machine evidence only. Real-child enjoyment, learning, retention, and acceptance were not performed or claimed. Natural-use Observation remains pending until real family-use evidence naturally exists.\n`, "utf8");
