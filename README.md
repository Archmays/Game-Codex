...
Portfolio 与 `allGameDefinitions` 保留 9 个可挂载定义；经典大厅当前展示 7 个独立入口。默认入口 `/` 是儿童侧“我的游戏世界”，其中可进入墨迹森林、数学世界和游戏百宝箱。
