...
export type M4OverlayKind = "none" | "spellbook" | "parent" | "repair";
...
export function renderM4Camp(state: M3GameState, progress: M4SaveState): string {
  ...
  return `<section class="hm2-panel hm2-camp hm2-m3-camp hm2-m4-camp" data-testid="chapter-one-m3-camp" data-m4-camp data-repair-count="${repaired.size}">
    ...
    <div class="hm2-camp-world" role="group" aria-label="八个持久营地修复">${M4_REPAIR_OBJECTS.map((repair, index) => {
      ...
      return `<button type="button" class="hm2-repair-object${ready ? " is-repaired" : ""}" style="--repair-index:${index}" data-action="open-repair" data-repair-id="${repair.id}" data-repaired="${String(ready)}" aria-label="${repair.name}，${ready ? "已经修好" : "等待字光"}"><span class="hm2-repair-shape" aria-hidden="true">${repair.icon}</span><b>${repair.name}</b><small>${ready ? repair.afterFunction : repair.beforeFunction}</small></button>`;
    }).join("")}</div>
    <div class="hm2-camp-portals"><button type="button" data-action="open-spellbook" class="hm2-spellbook-door"><span aria-hidden="true">▤</span><b>进入魔法书屋</b><small>${progress.discoveredCharacterIds.length}/36 道字光已遇见</small></button><button type="button" data-action="open-parent" class="hm2-parent-door">家长区</button></div>
    <div class="hm2-hero-grid" role="group" aria-label="三位英雄">${M3_HEROES.map((hero) => `<button type="button" class="hm2-hero-card${state.heroId === hero.id ? " is-selected" : ""}" data-action="select-hero" data-hero-id="${hero.id}" aria-pressed="${String(state.heroId === hero.id)}"><span data-icon-key="${hero.iconKey}" aria-hidden="true"></span><b>${hero.name}</b><small>${hero.shortDescription}</small><em>${hero.innateName}</em></button>`).join("")}</div>
...
export function renderM4Spellbook(progress: M4SaveState, view: M4SpellbookView): string {
  ...
  return `<section class="hm2-panel hm2-book" role="dialog" aria-modal="true" aria-labelledby="hm2-book-title" data-testid="chapter-one-spellbook" data-total-entries="36" data-filtered-count="${filtered.length}" data-page="${page + 1}" data-page-count="${pageCount}">
    ...
    ${selected && selectedCharacter ? `<article class="hm2-book-detail" data-testid="spellbook-detail" data-character-id="${selected.characterId}" data-replay-kind="${view.replayKind ?? "none"}"><div class="hm2-book-glyph"><span>${selected.glyph}</span><small>${selected.pinyinWithToneMarks}</small></div><div><h3>${selected.familiarWord}</h3><p>${selected.shortMeaning}</p><p><b>${structureLabel(selected.structure)}：</b>${selected.componentGlyphs.join(" ＋ ")} → ${selected.glyph}</p><p><b>${selected.magicName}：</b>${selected.magicEffect}</p><p><b>世界关联：</b>${selected.worldAssociation}</p><small>${selected.meaningImageDisclaimer}</small><div class="hm2-replay-actions"><button type="button" data-action="replay-pronunciation" data-character-id="${selected.characterId}">朗读“${selected.spokenPhrase}”</button><button type="button" data-action="replay-formation" data-character-id="${selected.characterId}">重看合字</button><button type="button" data-action="replay-meaning" data-character-id="${selected.characterId}">重看字义魔法</button></div>${view.replayKind === "formation" ? `<div class="hm2-formation-replay" aria-live="polite">${selected.componentGlyphs.map((glyph) => `<span>${glyph}</span>`).join("<b>＋</b>")}<b>→</b><strong>${selected.glyph}</strong></div>` : view.replayKind === "meaning" ? `<div class="hm2-magic-replay" data-meaning-asset-key="${selected.meaningAssetKey}" aria-live="polite"><i aria-hidden="true"></i>${selected.magicEffect}</div>` : view.replayKind === "pronunciation" ? `<div class="hm2-pronunciation-replay" aria-live="polite">正在重听：${selected.spokenPhrase}</div>` : ""}</div></article>` : ""}
...
export function renderM4RepairDetail(progress: M4SaveState, repairId: M4RepairId): string {
  ...
  return `<section class="hm2-panel hm2-repair-detail" role="dialog" aria-modal="true" aria-labelledby="hm2-repair-title" data-testid="chapter-one-repair-detail" data-repair-id="${repair.id}" data-repaired="${String(ready)}"><div class="hm2-overlay-heading"><div><p class="hm2-kicker">营地里的持久变化</p><h2 id="hm2-repair-title">${repair.name}</h2></div><button type="button" data-action="close-overlay">回到营地</button></div><div class="hm2-repair-compare"><div data-state="before"><span>${repair.icon}</span><b>修复前 · ${repair.beforeColor}</b><p>${repair.beforeShape}，${repair.beforeFunction}。</p></div><div data-state="after" class="${ready ? "is-visible" : ""}"><span>${repair.icon}</span><b>修复后 · ${repair.afterColor}</b><p>${repair.afterShape}，${repair.afterFunction}。</p></div></div><p>${ready ? "这份修复已经保存在本机。休息多久都不会退步。" : "继续发现完整汉字，这里会自然恢复；不需要金币或每日任务。"}</p><dl><div><dt>给孩子的变化</dt><dd>${repair.childValue}</dd></div><div><dt>汉字学习联系</dt><dd>${repair.hanziLearningValue}</dd></div></dl></section>`;
}

export function renderM4Parent(progress: M4SaveState, clearArmed: boolean, readOnly: boolean): string {
  return `<section class="hm2-panel hm2-parent" role="dialog" aria-modal="true" aria-labelledby="hm2-parent-title" data-testid="chapter-one-parent" data-clear-armed="${String(clearArmed)}"><div class="hm2-overlay-heading"><div><p class="hm2-kicker">家长区 · 本机摘要</p><h2 id="hm2-parent-title">隐私与本地进度</h2></div><button type="button" data-action="close-overlay">关闭</button></div><p>无账号、无上传、无广告；只保存发现字、八个修复、英雄、设置、当前安全路线、已见能力和聚合事件。不会保存详细按键历史。</p><dl><div><dt>发现字</dt><dd>${progress.discoveredCharacterIds.length}/36</dd></div><div><dt>营地修复</dt><dd>${progress.repairedObjectIds.length}/8</dd></div><div><dt>完成冒险</dt><dd>${progress.minimalLocalEvents.completedRuns}</dd></div><div><dt>存档状态</dt><dd>${readOnly ? "较新版本，只读保护" : progress.migration.source === "v1-schema-4" ? "已保留并迁移 V1" : "V2 本地存档"}</dd></div></dl><div class="hm2-clear-zone"><p>${clearArmed ? "再次确认会清除 V2 第一章本地进度；V1 冻结存档与旧路由不会被删除。" : "清除需要两次明确确认，避免误触。"}</p><button type="button" data-action="${clearArmed ? "confirm-clear-progress" : "arm-clear-progress"}" ${readOnly ? "disabled" : ""}>${clearArmed ? "再次确认清除" : "清除本地进度"}</button>${clearArmed ? `<button type="button" data-action="cancel-clear-progress">取消</button>` : ""}</div></section>`;
}
