...
if (JSON.stringify(GAME_PORTFOLIO.map(record => record.id).sort()) !== JSON.stringify(["hanzi-stroke-lab", "hanzi-word-adventure", "hanzi-tower-defense", "equation-slider", "math-lab", "make-target", "memory-card"].sort())) issues.push("portfolio retirement inventory");
...
if (KNOWN_SAVE_KEYS.length !== 47 || EXPORTABLE_SAVE_KEYS.length !== 46 || portfolioNamespacesWithoutKnownKey().length) issues.push("save key inventory");
