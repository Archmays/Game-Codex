...
describe("game portfolio governance", () => {
  ...
  it("represents the live catalog one-to-one with valid governance identities", () => {
    ...
    expect(GAME_PORTFOLIO.map(record => record.id)).toEqual(["oddity-puzzles", "world-in-a-box", "hanzi-stroke-lab", "english-study", "hanzi-word-adventure", "hanzi-tower-defense", "equation-slider", "math-lab", "make-target", "memory-card"]);
    ...
    expect(currentClassicGameCatalog).toHaveLength(5);
  ...
  it("keeps generated portfolio documents byte-deterministic and drift-free", () => {
    ...
    expect(status).toContain("Active child products：`5`");
  ...
  it("binds the terminal lifecycle, first-use surfaces, and exact save inventory", () => {
    ...
    expect(PRIMARY_PLAY_SURFACES.map((surface) => surface.id)).toEqual([
      "oddity-puzzles", "world-in-a-box", "hanzi-word-adventure", "my-game-world", "classic-hub", "hanzi-tower-defense", "math-world",
    ]);
