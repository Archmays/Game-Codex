...
async driver=>{
 ...
 await choose('riverside_trees','river');await p.waitForFunction(()=>document.querySelector('.wb-mount').dataset.follow===''&&document.querySelector('.wb-mount').dataset.boatRunning==='true');assert((await state()).follow==='','selection releases follow without stopping boat');await press('[data-slot=riverside_trees]');
 ...
 if(profile.name==='desktop')await p.screenshot({path:'tmp/tasks/world-box-step04/reset-city.png',fullPage:true});
 assert(errors.length===0,'no uncaught errors');rows.push({profile:profile.name,checks,diagnostics,errors});
 }catch(error){await p.screenshot({path:'tmp/tasks/world-box-step04/interactions-failure-'+profile.name+'.png',fullPage:true});return{result:'AUTO_REVISE',profile:profile.name,error:String(error),state:await state(),checks,diagnostics,rows,errors};}finally{await ctx.close();}
 }
