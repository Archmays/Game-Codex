...
export function mountBoxes(context:MountGameContext):MountedGame{
  ...
  async function open(scene:string){
    document.title=scene==='chancellor-desk'?'世界盒子 · 宰相的议事桌':scene==='dresden-river-campus'?'世界盒子 · 德累斯顿':'世界盒子 · 窗边有风';
    ...
    try{
      const module=scene==='chancellor-desk'?await import('./chancellor-runtime'):scene==='dresden-river-campus'?await import('./dresden-runtime'):await import('./runtime');
      if(dead||token!==generation)return;
      active='mountChancellor' in module?module.mountChancellor(context):'mountDresden' in module?module.mountDresden(context):module.mountWorldBox(context);
      ...
      dialog.innerHTML='<h2>打开哪个盒子？</h2><button data-box="window-breeze">01 · 窗边有风</button><button data-box="dresden-river-campus">02 · 德累斯顿</button><button data-box="chancellor-desk">03 · 宰相的议事桌</button><button data-close-boxes>继续玩</button>';root.querySelector('main')?.append(dialog);
  ...
  const values=new URLSearchParams(location.search).getAll('scene');const scene=values.length===0?'window-breeze':values.length===1&&['window-breeze','dresden-river-campus','chancellor-desk'].includes(values[0])?values[0]:'unknown';void open(scene);
