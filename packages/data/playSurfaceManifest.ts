...
export const PLAY_SURFACE_MANIFEST: readonly PlaySurfaceRecord[] = [
  ...
  ...(["assemble", "tone", "contrast"] as const).map((mode) => hanzi({ id: `hanzi-pinyin-${mode}`, title: `声韵试炼 · ${mode}`, route: `?play=hanzi-magic-complete&view=pinyin&mode=${mode}`, kind: "support-activity", parentSurfaceId: "hanzi-world", returnRoute: "?play=hanzi-magic-complete&from=hub", primaryActionSelector: "[data-answer], [data-action=hint], a" })),
  ...(["same-glyph", "glyph-pinyin", "glyph-phrase"] as const).map((pack) => hanzi({ id: `hanzi-memory-${pack}`, title: `字光配对 · ${pack}`, route: `?play=hanzi-magic-complete&view=memory&pack=${pack}`, kind: "support-activity", parentSurfaceId: "hanzi-world", returnRoute: "?play=hanzi-magic-complete&from=hub", primaryActionSelector: "[data-card-id], [data-pack-id], a" })),
