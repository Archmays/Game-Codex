...
| 36 字正式内容管线 | 合字始终是施法核心 | hanzi-structure-quality、puzzle | 字量扩展仍保持熟悉词、清楚结构和可撤回操作 | 14 左右、12 上下、5 全包围、5 半包围；每字来源、拼音、短义、魔法与唯一解手牌绑定 | 36/36 schema、母库/公式/视觉账本、108 手牌全量求解、6 张结构与完整字截图、5,000 coverage seeds | 未来可选逐字理解观察，不是本轮门禁 | `M2_PASS_STAGE / MACHINE_CONTENT_VERIFIED` |
| 三英雄与能力构筑 | 自主选择带来重玩而不靠压力机制 | Slay the Spire、Brotato、Dicey Dungeons、child-first-learning-game | 三位英雄和每局三次三选一形成可理解的不同玩法 | 能力只支持放置、恢复、提示和字义反馈，不改正确答案 | 3 固有+18 可选 exact IDs、候选生成、触发状态、UI 标记、30,000 coverage seeds、9 条浏览器通关 | 未来可选偏好观察，不推断普适偏好 | `M3_PASS_STAGE / MACHINE_BUILD_VERIFIED` |
...
| 三区域首领与最终字核 | 完整章节高潮只综合已教规则 | level-design、game-feel | 3 个区域高潮与三阶段最终恢复形成清楚起承转合 | 最后一击由完整汉字发出；最终首领不引入未见结构规则 | 4 首领可达/可完成、intent、phase restore、browser playthroughs、ending state | 未来可选情绪和理解观察 | `M5_PASS_STAGE / MACHINE_FLOW_VERIFIED` |
| 八项营地修复与 36 页魔法书 | 世界永久变化和低压收藏支持回归 | save-systems、DragonBox、Metamorphabet | 不用每日任务也能看见属于自己的变化与发现 | 每页重现字、拼音、词、短义、结构、部件和字义魔法 | 8 repair keys、36 entries、三类 replay、搜索/分页/过滤、三 viewport 与 6 张截图 | 未来可选自发回看观察 | `M4_PASS_STAGE / MACHINE_PERSISTENCE_VERIFIED` |
| V2 本地存档迁移 | 进度连续且隐私最小 | save-systems | V1 发现与修复不丢失，损坏时温和恢复 | 保留 12 字与精确 3 修复；扩展到 36 字和 8 修复 | raw V1 bytes、schema/checksum/backup/recovery/future read-only/current-run reload/二次确认 clear | 家长可解释并二次确认清除 | `M4_PASS_STAGE / V1_MIGRATION_VERIFIED` |
| Theme C 第一章正式视觉与音频 | 反馈明确、静音和减少动画仍完整 | game-feel、audio-design、imagegen | 英雄、区域、怪物和修复形成同一儿童安全世界 | 结构和完整汉字始终由代码清晰渲染，不让生成图承担字形正确性 | 72 资产 manifest/hash/WebP/预算、30 状态 exact PNG/ARIA baseline、mute/reduced motion | 未来可选审美偏好，不宣称已偏好 | `M5_PASS_STAGE / THEME_C_BOUND` |
...
| V2.0 第一章本地启动器 | 双击即进入且不干扰其他进程 | save-systems、machine-first-game-review | 家庭无需命令行即可回到同一完整章节 | launcher 只承载 canonical runtime，不复制内容或存档 | PID/start-time/root/command/port identity、HTTP readiness、reuse、owned cleanup、port fallback | 家庭可直接双击游玩；非儿童验证 | `M5_PASS_STAGE / LIFECYCLE_VERIFIED` |
