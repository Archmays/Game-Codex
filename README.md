...
《世界盒子》v0.1.0 首个八件拼装样板《窗边有风》已接入大厅，入口 `?play=world-in-a-box`；[玩法与制作](games/world-in-a-box/README.md)，[本轮验证报告](docs/world-in-a-box/step01-r2-report.md)。

## 当前中文游戏

《字间行者》v1.0.0 包含「借字归途」「借一盏明」「光与归途」「结伴归途」四章共二十房，用搬字、林木拆合、光、局部否定与双角色分工改变通路，入口 `?play=hanzi-word-adventure`，见 [游戏说明](games/hanzi-word-adventure/README.md)。《字阵守城》v1.0.0 保留三图 22 波及六个三波短局，入口 `?play=hanzi-tower-defense`，见 [游戏说明](games/hanzi-tower-defense/README.md)。本轮美术、声音、可靠性与发布验证见 [v1.0 报告](docs/v1.0-release-report.md)。墨迹森林全线和词光岛全线已退役，旧链接只回现有首页；历史通过 Git 保留，旧语言存档原值保留，数学玩法与存档不变。
...
`allGameDefinitions` 保留 10 个可挂载定义，但儿童产品组合已明确收敛为 5 个活跃产品；经典大厅只投影这 5 个产品。另有 2 个独立工具；世界模块、兼容入口和共享引擎分别维护，不再拿定义数冒充产品数。

| 游戏 | 学科 | 目标世界 | 定义角色 | 活跃儿童产品 | 质量等级 | 当前状态 |
| --- | --- | --- | --- | --- | --- | --- |
| 奇物事务所 | 观察与推理 | 共享模块 | 活跃儿童产品 | 是 | B | 三关试用版 |
| 世界盒子 | 空间与观察 | 共享模块 | 活跃儿童产品 | 是 | B | 三个可玩盒子 |
| 汉字书房 | 识字工具 | 中文世界 | 独立查阅工具 | 否 | A | 可用 |
| 英语书房 | 英语工具 | 英语世界 | 独立查阅工具 | 否 | A | 可用 |
| 字间行者 | 识字 | 中文世界 | 活跃儿童产品 | 是 | S | 可玩 |
...
当前减法：`docs/portfolio-retirement-step1.md`；时钟、阵列、旧实验室已退役，旧链接回数学世界，全部旧存档键保留。
家庭使用入口、可选 Observation Kit 与重新开发边界见 `docs/project-status/natural-use.md`。
<!-- GAME_PORTFOLIO:END -->

> **算式滑轨 V3：**4 章共 200 关已按新 schema 重建；正式棋盘教程、统一状态机、40 个手工金标准、确定性生成、真实 pointer/键盘 E2E 与 25 关 UI-only agent 试玩均已纳入发布门禁。它现在只作为数学世界 S 级旗舰模块进入；当前组合真值见 `docs/portfolio-retirement-step1.md`，关卡验收与后续儿童观察边界见 `docs/equation-slider/rebuild-v3/`。
...
- `?hub=classic`：3 个正式世界的备用入口；算式滑轨、目标工坊归入数学世界，记忆配对归入中文/英语世界；
- `?world=math-world`：数学世界 · 数感实验城；支持 `station=slider|target` 直接进入并刷新恢复；`station=lab|clock|array` 只显示收起说明并回到地图，保留旧存档；
