...
test("@workbench all 46 exact keys survive the public Vault roundtrip and the mounted old page cannot overwrite restored records", async ({ page, context }, info) => {
  expect(KNOWN_SAVE_KEYS).toHaveLength(46);
  expect(EXPORTABLE_SAVE_KEYS).toHaveLength(45);
  await page.goto(origin);
  ...
  await expect(vaultPage.locator("[data-vault-status]")).toHaveText("已恢复 45 个已知存档；0 个未知键已跳过。请刷新页面查看。");
