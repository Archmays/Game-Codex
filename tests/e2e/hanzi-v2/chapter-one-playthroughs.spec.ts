...
import { computeHanziV2SourceTreeSha256 } from "../../../tools/hanzi-v2-chapter-one/source-identity";

const root = resolve("test-results/hanzi-v2/chapter-one");
const reportDataDir = resolve(root, "report/data");
...
const sourceTreeSha256 = process.env.CHAPTER_ONE_SOURCE_TREE_SHA256 ?? computeHanziV2SourceTreeSha256();
...
test("M5 18-run release matrix completes without machine errors", async ({ browser }, testInfo) => {
  ...
  writeFileSync(resolve(reportDataDir, "MACHINE-PLAYTHROUGH-MATRIX.json"), `${JSON.stringify(matrix, null, 2)}\n`, "utf8");
