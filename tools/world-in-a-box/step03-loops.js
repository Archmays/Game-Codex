...
async driver=>{
 ...
 for(const scene of ['window-breeze','dresden-river-campus']){
 ...
 await p.waitForFunction(()=>JSON.parse(document.querySelector('.wb-mount').dataset.audio).loops.some(l=>l.key==='music'));
 const initial=await p.locator('.wb-mount').evaluate(e=>JSON.parse(e.dataset.audio).loops.find(l=>l.key==='music'));
 const duration=scene==='window-breeze'?75.78945833333333:91.42858333333334;
 ...
 const final=await p.locator('.wb-mount').evaluate(e=>JSON.parse(e.dataset.audio).loops.find(l=>l.key==='music'));
