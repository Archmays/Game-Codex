...
function gitPaths(workspaceRoot: string): string[] {
  return execFileSync("git", ["ls-files", "--cached", "--others", "--exclude-standard", "-z"], { cwd: workspaceRoot, encoding: "utf8" })
    .split("\0")
...
export function computeHanziCompleteSourceTreeSha256(workspaceRoot = process.cwd()): string {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  process.stdout.write(`${computeHanziCompleteSourceTreeSha256(resolve(process.argv[2] ?? process.cwd()))}\n`);
}
