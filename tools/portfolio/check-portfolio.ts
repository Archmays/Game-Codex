...
export function validatePortfolio(catalog: readonly GameCatalogMetadata[], currentCatalog: readonly GameCatalogMetadata[], root = resolve(import.meta.dirname, "../..")): string[] {
  const issues: string[] = [];
  const expectedIds = ["world-in-a-box", "hanzi-stroke-lab", "hanzi-word-adventure", "hanzi-tower-defense", "equation-slider", "math-lab", "make-target", "memory-card"].sort();
  ...
  for (const record of GAME_PORTFOLIO) {
    ...
    if (record.contentStatus !== (["world-in-a-box", "hanzi-stroke-lab", "hanzi-tower-defense", "hanzi-word-adventure"].includes(record.id) ? "playable" : "frozen")) issues.push(`${record.id} content status differs from the authorized new-game / protected-math boundary`);
  ...
  const expectedActiveProducts = ["hanzi-tower-defense", "hanzi-word-adventure", "math-lab", "world-in-a-box"];
  if (JSON.stringify(ACTIVE_CHILD_PRODUCTS.map((record) => record.id).sort()) !== JSON.stringify(expectedActiveProducts)) issues.push("Active child product truth must contain the four current products");
  if (JSON.stringify(CLASSIC_CARD_PRODUCTS.map((record) => record.id).sort()) !== JSON.stringify(expectedActiveProducts)) issues.push("Classic Hall must project only the four current products");
