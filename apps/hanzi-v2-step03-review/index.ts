...
export function mountHanziV2Step03Review(root: HTMLElement): void {
  ...
  const panelFor = (): string => {
    ...
    if (activeTab === "manifest") {
      return `<section class="step03-panel"><header class="step03-heading"><div><span class="step03-kicker">12 STABLE CHARACTER RECORDS</span><h2>12 字 Manifest</h2><p>每字有独立 stable ID、revision hash、决定和反馈。changed-only 导入时，已 ACCEPT 且未变的字折叠；REVISE、REJECT、新字或变更字仍展开。</p></div><div class="step03-count">${FINAL_GOLDEN_MANIFEST.length} / 12</div></header><div class="step03-manifest-grid">${FINAL_GOLDEN_MANIFEST.map((character) => { const decision = characterDecision(character.id); return `<article class="step03-character" data-testid="final-manifest-card-${character.id}" data-character-review="${character.id}"><div><strong lang="zh-Hans">${character.glyph}</strong><span>${escapeHtml(character.visualPinyin)}</span></div><h3>${escapeHtml(character.familiarWord)}</h3><p>${escapeHtml(character.shortMeaning)}</p><dl><div><dt>真实结构</dt><dd>${escapeHtml(character.structure)} · ${character.components.map((part) => `${part.glyph}→${part.slotId}`).join("；")}</dd></div><div><dt>字义魔法</dt><dd>${escapeHtml(character.magic.name)}：${escapeHtml(character.magic.effect)}</dd></div><div><dt>范围</dt><dd>${character.stage === "first-run" ? "首次 run" : "成人 manifest 浏览"}</dd></div></dl>${decision.carriedForward ? `<div class="step03-character-carried" data-carried-forward="character:${character.id}">已沿用上一轮 ACCEPT</div>` : `${individualDecisionButtons("data-character-decision", character.id, decision.decision)}<label>必填反馈<textarea rows="2" data-character-notes="${character.id}">${escapeHtml(decision.notes)}</textarea></label>`}<small>${character.id} · ${escapeHtml(decision.revisionHash)}</small></article>`; }).join("")}</div>${itemEvidence("final-manifest")}${notesOnly("final-manifest", "12 字 Manifest 粗粒度决定")}</section>`;
    }
