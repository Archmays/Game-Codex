...
export class Handwriting {
  ...
  private active=false;private matches:Match[]|null=null;
  ...
  start():void {
    ...
    this.active=true;
    if(this.worker){this.restore();return;}
    ...
    this.worker.onmessage=({data})=>{
      ...
      if(data.type==='ready'){this.ready=true;if(this.active)this.restore();return;}
      if(data.revision!==undefined&&data.revision!==this.revision)return;
      if(data.type==='error'){this.status(data.message);return;}
      if(data.type==='result'&&this.active&&this.strokes.length){this.wanted=false;this.matches=data.matches;this.restore();}
    };
    this.worker.onerror=()=>{if(this.destroyed||this.worker!==worker)return;this.status('手写模型暂不可用，请点“重新载入识别”。');worker.terminate();this.worker=null;this.ready=false;};
  }
  private restore():void {
    if(this.matches!==null){this.candidates(this.matches);this.status(this.matches.length?`${this.strokes.length} 笔 · 请选择候选，或重新识别。`:'没有找到候选，可重新识别或调整笔迹。');}
    else if(this.strokes.length)this.schedule();
    else this.status(this.ready?'写一个字，停笔后选候选。':'正在载入本地手写模型……');
  }
  retry():void {this.invalidate();this.worker?.terminate();this.worker=null;this.ready=false;this.start();}
  recognize():void {this.invalidate();this.start();}
  invalidate(clear=true):void {this.revision++;this.wanted=false;clearTimeout(this.timer);if(clear){this.matches=null;this.candidates([]);}}
  private schedule():void {
    ...
    if(this.ready&&this.active)this.timer=window.setTimeout(()=>{
      if(revision===this.revision&&!this.destroyed&&this.active)this.worker?.postMessage({revision,strokes:normalizeStrokes(this.strokes)});
    },220);
  }
  ...
  cancel():void {const incomplete=Boolean(this.pending);this.pending=null;this.down=false;this.release();if(incomplete){this.invalidate();if(this.active&&this.strokes.length)this.schedule();else this.status('未完成的一笔已取消；已写笔画保留。');}this.redraw();}
  suspend():void {this.active=false;this.cancel();this.invalidate(false);this.keyboard=false;this.redraw();if(this.matches===null&&this.strokes.length)this.status('笔迹已保留，返回手写区后重新识别。');}
  undo():void {this.cancel();this.invalidate();this.strokes.pop();this.redraw();this.changed();this.schedule();}
  clear():void {this.cancel();this.invalidate();this.strokes.length=0;this.redraw();this.changed();this.status('已清空，可以重新写一个字。');}
