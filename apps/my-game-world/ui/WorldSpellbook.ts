...
export function mountWorldSpellbook(
  ...
  const render = (): void => {
    const active = activeId ? getGoldenCharacter(activeId) : null;
    root.innerHTML = `<section class="world-modal" role="dialog" aria-modal="true" aria-labelledby="world-spellbook-title" data-testid="world-spellbook">
      <div class="world-modal__panel world-spellbook">
        ...
        <h2 id="world-spellbook-title">${WORLD_COPY.spellbookTitle}</h2>
        ${entries.length ? `<nav aria-label="已经发现的字" class="world-spellbook__tabs">${entries.map((id) => {
          const character = getGoldenCharacter(id);
        ...
        ${active ? `<article class="world-spellbook__page" data-testid="world-spellbook-page-${active.id}">
          ...
          <div><h3>${active.visualPinyin} · ${active.familiarWord}</h3><p>${active.shortMeaning}</p><p>${active.components.map((part) => part.glyph).join(" + ")} → ${active.glyph}</p><p>${active.magic.effect}</p><button type="button" data-world-read-character="${active.id}">读给我听</button></div>
        </article>` : ""}
    ...
    root.querySelector<HTMLElement>("[data-world-read-character]")?.addEventListener("click", () => {
      if (!activeId) return;
      void audio.speak(getGoldenCharacter(activeId).spokenPhrase);
    });
