...
export function affectedGateCommands(changedFiles: readonly string[]): GateCommand[] {
  ...
  if (full) return [PORTFOLIO_CHECK, INTERACTION_STATIC, UNIT, MATH_WORLD_VALIDATE, CHINESE_SUPPORT_VALIDATE, TYPECHECK, BUILD, MATH_WORLD_E2E, CHINESE_SUPPORT_E2E, ENGLISH_E2E, INTERACTION_HITTEST, SMOKE];
  const commands: GateCommand[] = [PORTFOLIO_CHECK];
  for (const file of files) {
    const game = /^games\/([^/]+)\//.exec(file)?.[1];
    if (/^(?:apps|games|packages|src)\/.+\.css$/.test(file)) commands.push(INTERACTION_STATIC, INTERACTION_HITTEST);
    ...
    else if (game === "english-spell-battle") commands.push(INTERACTION_STATIC, ENGLISH_E2E, INTERACTION_HITTEST, SMOKE);
    ...
    else if (file.startsWith("apps/hub/") || file.startsWith("packages/ui/") || file.startsWith("packages/data/") || file.startsWith("public/assets/")) commands.push(INTERACTION_STATIC, UNIT, MATH_WORLD_VALIDATE, INTERACTION_HITTEST, SMOKE);
