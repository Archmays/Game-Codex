...
test("Classic retains memory and has six cards while the legacy Pinyin definition is hidden", async ({ page }) => {
  await page.goto("/?hub=classic");
  await expect(page.locator(".game-card")).toHaveCount(6);
  await expect(page.locator('[data-game-id="memory-card"]')).toBeVisible();
  await expect(page.locator('[data-game-id="pinyin-magic-battle"]')).toHaveCount(0);
});
