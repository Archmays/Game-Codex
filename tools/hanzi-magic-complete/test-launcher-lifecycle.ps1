...
param(
  ...
  [string]$SourceTreeSha256 = $env:HANZI_COMPLETE_SOURCE_TREE_SHA256
)
...
$startScript = Join-Path $PSScriptRoot "START_HANZI_MAGIC_BATTLE_COMPLETE.ps1"
$stopScript = Join-Path $PSScriptRoot "STOP_HANZI_MAGIC_BATTLE_COMPLETE.ps1"
$reportPath = Join-Path $repositoryRoot "test-results\hanzi-complete\launcher\LAUNCHER-LIFECYCLE.json"
...
try {
  ...
  $first = Read-CompleteServerRecord -Paths $paths
  ...
  & $stopScript
  if (@(Get-CompleteListenerProcessIds -Port ([int]$first.port)).Count -ne 0) { throw "Lifecycle stop left the owned port listening." }
  ...
  Write-CompleteUtf8NoBom -Path $reportPath -Contents (($result | ConvertTo-Json -Depth 6) + "`n")
}

if ($result.result -ne "PASS") { throw "Complete Edition launcher lifecycle did not pass." }
