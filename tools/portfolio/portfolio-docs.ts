...
export function renderReadmePortfolio(catalog: readonly GameCatalogMetadata[]): string {
  ...
  return [
    ...
    ...rows,
    "",
    `当前两个产品领域（${PRIMARY_WORLDS.join(" / ")}）包含 ${WORLD_MODULES.length} 个显式世界模块；${COMPATIBILITY_SURFACES.length} 个兼容表面与 ${SHARED_ENGINES.length} 个共享引擎独立登记。算式滑轨只作为数学世界旗舰模块进入；目标工坊与记忆配对也不再占用重复 Classic 卡，数学模块、独立配对引擎与原存档保留；旧语言产品已实际退役，旧链接仅返回首页。`,
    "",
    ...
    `历史周期 \`${currentCycle.id}\` 是用户明确授权、由发布 tag 目标闭合的独立 bounded development cycle；源码不提前冒充外部发布完成。它不关闭 Natural-use Observation，也不创建自动后续任务。`,
...
export function renderPortfolioStatus(catalog: readonly GameCatalogMetadata[]): string {
  ...
  return [
    ...
    "",
    `- 历史发布周期：\`${currentCycle.id}\`；完成条件：\`${currentCycle.completionCondition}\`；最终完成由发布 tag、CI 与 Pages 同 SHA 回读证明，不由源码预先宣告。`,
    `- 当前收敛阶段：\`${ACTIVE_PROJECT_PHASE}\`` ,
    `- 下一自动阶段：\`${NEXT_PROJECT_PHASE ?? "NONE"}\`` ,
    `- 当前两个产品领域：\`${PRIMARY_WORLDS.join(" / ")}\`` ,
