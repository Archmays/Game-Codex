...
import type { M3AbilityEvidence, M3AbilityEffects, M3Action, M3GameState, M3InnateEvidence, M3PathPlan, M3SimulationResult } from "./m3-types";
...
function triggerAbilities(state: M3GameState, trigger: M3AbilityTrigger, context: { placedSourceGlyph?: string; wasSeenBefore?: boolean } = {}): M3GameState {
  ...
  for (const abilityId of state.selectedAbilityIds) {
    ...
    next = {
      ...
      abilityEvidence: updateEvidence(next, abilityId, {
        triggered: true,
        stateChanged: abilityEffects[ability.effectKey] !== before,
        visibleEffectObserved: true,
      }),
    };
  }
...
function loadEncounter(state: M3GameState, index: 0 | 1 | 2 | 3): M3GameState {
  ...
  const encounter = path.encounters[index];
  ...
  let next: M3GameState = {
    ...
    gentleMessage: `${getM1Behavior(encounter.behaviorId).name}要来了，${character.structure === "left-right" ? "左右" : character.structure === "top-bottom" ? "上下" : "包围"}位置仍然真实。`,
  };
  if (encounter.boss) next = triggerAbilities(next, "boss-telegraph");
  return next;
}

export function createM3GameState(seed = "ink-forest-2", heroId: M3HeroId = "light-speaker"): M3GameState {
  const plan = generateM3RunPlan(seed, heroId);
  ...
  return {
    ...
    gentleMessage: "选一位伙伴，三条路都能抵达第一章的字光。",
...
function transitionAfterMeaning(state: M3GameState): M3GameState {
  if (state.encounterIndex === 2) return { ...state, phase: "ability-choice", gentleMessage: "选一道会在本区首领战真正发生的字光。" };
  if (state.encounterIndex === 3) return { ...state, phase: "region-complete", gentleMessage: `${state.plan.regions[state.regionIndex].title}重新亮起来了。` };
...
export function reduceM3State(state: M3GameState, action: M3Action): M3GameState {
  if (action.type === "repeat-seed" && state.phase === "run-summary") return { ...createM3GameState(state.seed, state.heroId), actionCount: state.actionCount + 1 };
  if (action.type === "select-hero" && state.phase === "camp") {
    const fresh = createM3GameState(state.seed, action.heroId);
  ...
  if (action.type === "recover-behavior" && state.phase === "behavior-effect" && state.currentEncounter) {
    let next = incrementAction(state, { phase: "encounter", currentBehaviorRecovered: true, completedBehaviorCycles: [...state.completedBehaviorCycles, state.currentEncounter.behaviorId], gentleMessage: "把两块真正的字灵送回结构位置。" });
  ...
  if (action.type === "place-card" && state.phase === "encounter" && state.currentEncounter) {
    ...
    const protectedPlacement = firstPlacement && (state.heroId === "ink-companion" || state.selectedAbilityIds.includes("root-guard"));
    const placement = { cardId: card.id, slotId: action.slotId, protected: protectedPlacement } as const;
    const placements = [...state.placements, placement];
    let next = incrementAction(state, {
      placements,
      selectedCardId: null,
      seenComponentGlyphs: unique([...state.seenComponentGlyphs, card.sourceGlyph]),
      gentleMessage: `${card.glyph}稳稳落在真实位置上。`,
    });
    ...
    if (state.heroId === "forest-speaker" && state.seenComponentGlyphs.includes(card.sourceGlyph)) next = markInnate(next, { ...next.heroEffects, growthLinkCount: next.heroEffects.growthLinkCount + 1 });
    ...
    next = triggerAbilities(next, "first-correct-placement", { placedSourceGlyph: card.sourceGlyph, wasSeenBefore: state.seenComponentGlyphs.includes(card.sourceGlyph) });
  ...
  if (action.type === "continue" && state.phase === "composition") {
    let next = incrementAction(state, { phase: "meaning", gentleMessage: "完整汉字正在变成它自己的字义魔法。" });
  ...
  if (action.type === "choose-ability" && state.phase === "ability-choice") {
    ...
    let next = incrementAction(state, {
      selectedAbilityIds: [...state.selectedAbilityIds, action.abilityId],
      abilityEvidence: updateEvidence(state, action.abilityId, { selected: true }),
      gentleMessage: `${getM3Ability(action.abilityId).name}已经成为本局第 ${state.selectedAbilityIds.length + 1} 枚字光。`,
    });
    next = triggerAbilities(next, "on-select");
    return loadEncounter(next, 3);
  }
  if (action.type === "continue" && state.phase === "region-complete") {
    if (state.regionIndex === 2) return incrementAction(state, { phase: "run-summary", gentleMessage: "三片区域都亮了。可以换英雄、换路线或沿同一 seed 重放。" });
...
export function replayM3Actions(seed: string, heroId: M3HeroId, actions: readonly M3Action[]): M3GameState {
  return actions.reduce(reduceM3State, createM3GameState(seed, heroId));
}

export function simulateM3Run(seed: string, heroId: M3HeroId): M3SimulationResult {
  let state = createM3GameState(seed, heroId);
  const actions: M3Action[] = [];
  const failureCodes: string[] = [];
  for (let guard = 0; guard < 500 && state.phase !== "run-summary"; guard += 1) {
    let action: M3Action;
  ...
  if (state.discoveredCharacterIds.length !== 12) failureCodes.push("not-12-unique-characters-cast");
  ...
  if (placementActions !== 24) failureCodes.push("ability-auto-solved-or-extra-placement");
...
export function currentM3Character(state: M3GameState) {
  return state.currentEncounter ? getChapterOneCharacter(state.currentEncounter.characterId) : null;
}
