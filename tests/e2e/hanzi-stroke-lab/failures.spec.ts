...
import {pane,details} from './helpers';
...
test('late character data, denied storage, missing model and retry are honest',async({page,context})=>{
 ...
 await page.locator('#hsl-input').fill('永水');await page.locator('[data-search] button').click();await details(page,1);await expect(page.locator('[data-step-status]')).toContainText('共 4 笔');await page.waitForTimeout(800);await expect(page.locator('#hsl-character-title')).toContainText('水');await expect(page.locator('[data-steps] button')).toHaveCount(4);await expect(page.locator('[data-storage-status]')).toContainText('原记录已保留');
 await page.route('**/recognizer/hanzi_lookup_bg.wasm',r=>r.abort());await page.reload();await page.locator('[data-index-ready=true]').waitFor();await pane(page,'hand');await expect(page.locator('[data-hand-status]')).toContainText('没有加载成功');await page.locator('summary').filter({hasText:'没有找到'}).click();await page.unroute('**/recognizer/hanzi_lookup_bg.wasm');await page.locator('[data-retry-hand]').click();await expect(page.locator('[data-hand-status]')).toHaveText('写一个字，停笔后选候选。');
});
test('pending handwriting clear, cancellation, multiple contacts and teardown invalidate work',async({page})=>{
 ...
 await page.goto('?play=hanzi-stroke-lab');await page.locator('[data-index-ready=true]').waitFor();await pane(page,'hand');const pad=page.locator('[data-pad]');
 ...
 await page.evaluate(()=>window.dispatchEvent(new PageTransitionEvent('pageshow',{persisted:true})));await page.locator('[data-index-ready=true]').waitFor();await page.locator('#hsl-input').fill('永');await page.locator('[data-search] button').click();await details(page);await expect(page.locator('[data-steps] button')).toHaveCount(5);
});
test('empty library has a visible failure state; small height and zoom remain navigable',async({page})=>{
 await page.route('**/hanzi-stroke-lab/index.json',r=>r.fulfill({json:{}}));await page.goto('?play=hanzi-stroke-lab');await page.locator('[data-index-ready=true]').waitFor();await page.locator('#hsl-input').fill('永');await page.locator('[data-search] button').click();await expect(page.locator('#hsl-query-note')).toContainText('尚未载入');await page.unroute('**/hanzi-stroke-lab/index.json');await page.reload();await page.locator('[data-index-ready=true]').waitFor();await page.setViewportSize({width:1024,height:480});await page.evaluate(()=>document.documentElement.style.zoom='1.5');await page.locator('#hsl-input').fill('鱻');await page.locator('[data-search] button').click();await details(page);await expect(page.locator('[data-steps] button')).toHaveCount(33);await page.locator('[data-steps] button').last().click();await expect(page.locator('[data-step-status]')).toContainText('33 / 33');expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1)).toBe(true);
});
