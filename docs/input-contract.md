...
| --- | --- | --- | --- |
| World Box `?play=world-in-a-box` | Three-item tray pages, ray-visible interfaces, camera, help/undo, installed shutters/cup/cat, scoped restart | Tab regions; arrows/Home/End within tray or visible interfaces; Enter/Space activate once; Escape cancels selection/dialog; placing restores next tray focus | Release confirms; select then tap visible interface or supplementary drag; hold left/right to rotate continuously (release/cancel stops), zoom buttons; real geometry raycasts reject occluded targets; gestures cancel on wheel, blur, hidden, multi-touch and pointercancel |
| World Box Dresden `?play=world-in-a-box&scene=dresden-river-campus` | Four tray regions, 18 arbitrary-order pieces, geometry-visible interfaces, tram destinations, boat/dock/follow, reversible SLUB section, six photo dialogs and clocks | Same regional navigation; Z undoes once; native transport/section controls; clock descriptions receive focus; photo Escape restores opener; no progress injection | Same select/tap and optional drag; hold left/right rotates continuously on mouse/touch/Enter/Space; moving anchors follow actual vehicles; nearby transport/section buttons remain visible below scene; scroll cancels an active gesture; background pauses motion; city reset preserves old v1 |
...
| Adventure `?play=hanzi-word-adventure` | World, moves, target, transformations, hints, save selection | Arrows/WASD move; Shift+arrows inspect; Space/E explicit take/place/preview confirm; X directly splits, C directly combines, Z undoes, H opens hints; preview arrows choose, Enter/Space confirm, Esc cancels/closes; Q switches actor only in companions | Adjacent walkable cell moves; objects inspect, main action performs; while carrying, Choose placement selects without walking; actor buttons switch; directions and main action remain together on phones |
| Tower `?play=hanzi-tower-defense` | Materials, tower positions, English equipment, preview, battle, 1×/2× speed, tactical scenarios | Region Tab; region arrows; native confirm, retry and speed buttons; P pause only within game; Esc cancel | Select entities then target/confirm; ranges inspect independently; speed changes battle time only; tactical deployed-tower moves/stow and English transfer are between waves, including while paused |
...
Adventure world actions continue without a Tab detour after take/place, transform, undo, hint close or room change. Preserve the accessible action nodes during ordinary renders. Restore focus according to the action's source: world Z and pointer undo return to the world; a deliberately Tab-reached undo button retains its native focus after activation. Likewise H/Esc returns to the world, while keyboard activation of the hint button restores that opener. A consumed confirmation must not reach the next room or underlying world. Q changes only the controlled actor and its corresponding view, without advancing wind or changing either actor's materials. Tactical retry returns the declared actual pre-wave checkpoint to preparation; it does not resume battle automatically.
