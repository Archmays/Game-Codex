...
async driver=>{
 ...
 // Full river traversal includes deceleration and lateral berthing; its physical duration can exceed the 9s UI timeout.
 await press('[data-action=boat-dock]');await p.waitForFunction(()=>document.querySelector('[data-boat-running]').dataset.boatRunning==='false'&&Math.abs(+document.querySelector('.wb-canvas').dataset.boat.split(',')[0]-5.8)<.0001,undefined,{timeout:14000});await p.waitForTimeout(700);check(+((await p.locator('.wb-canvas').getAttribute('data-boat')).split(',')[2])>-3.55,'boat docks alongside real installed pier');
